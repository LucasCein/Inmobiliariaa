...
const ComprobantesItems = ({ comprobantes }) => {
  ...
  return (
    <>
      {
        currentComprobnates.map(({ id, Fecha, Tipo, pTotal, nombreProveedor, idDetalle, idProp, idProv, nombrePropiedad, cuitProveedor, Suc, numComp }) =>
          <MDBListGroupItem key={id} className='container align-items-center justify-content-center ' >
            <div className="row ms-1">
              <div className='col'>
                <p className='text-dark mb-0'>{nombreProveedor}</p>
              </div>
              <div className="col">
              ...
              <div className="d-flex gap-2 col">
                ...
                <Popup className="position-absolute" trigger={<button className='btn btn-info '><BsEyeFill></BsEyeFill></button>} modal>
                  <DetalleComprobante item={{ id, Fecha, Tipo, pTotal, nombreProveedor, idDetalle, idProp, idProv, propiedadName, cuitProveedor, Suc, numComp }}></DetalleComprobante>
                </Popup>
              </div>
            </div>
