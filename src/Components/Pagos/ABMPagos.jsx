...
const ABMPagos = (detailData) => {
  ...
  const { data: proveedoresData } = useFirestoneCollection("proveedores", [
    ["Activo", "==", true],
  ]);

  useEffect(() => {
    const fetchprovee = async () => {
      const proveDeafult = proveedoresData.filter(
        (prove) => prove.value === detailData.factura.nombreProveedor
      );
  ...
  useEffect(() => {
    const fetchComprobantes = async () => {
      ...
      comprobantesData = comprobantesData.filter(
        (comprobante) => comprobante.idProv == proveedorSeleccionado.id
      );
      ...
      comprobantesData = comprobantesData.map((comprobante) => {
        const propiedad = propiedadList.find(
          (prop) => prop.id === comprobante.idProp
        );
        const comprobanteConPropiedad = {
          ...comprobante,
          nombrePropiedad: propiedad ? propiedad.nombre : "",
        };
        comprobantesData.filter(
          (comprobante) => comprobante.idProv == proveedorSeleccionado.id
        );
  ...
  const handleChange = (comprobanteCheq) => {
    if (
      comprobantesSeleccionados.some(
        (comprobante) => comprobanteCheq.id == comprobante.id
      )
    ) {
      const sacarComprobate = comprobantesSeleccionados.filter(
        (comp) => comp.id != comprobanteCheq.id
      );
      setComprobantesSeleccionados(sacarComprobate);
    } else {
      setComprobantesSeleccionados(
        comprobantesSeleccionados.concat(comprobanteCheq)
      );
    }
  };
  ...
  const createPag = (idDet) => {
    const pago = {
      ...
      monto: comprobantesSeleccionados.reduce((acc, com) => {
        return acc + com.pTotal;
      }, 0),
    };
      ...
      {comprobantesSeleccionados.length > 0 ? (
        <div>
          <div className="d-flex align-items-start justify-content-between m-1">
            <h3>
              Total: $
              {comprobantesSeleccionados.reduce((acc, com) => {
                return acc + com.pTotal;
              }, 0)}
            </h3>
          </div>
          ...
          <div className="d-flex align-items-center justify-content-center pt-3">
            <button className="btn btn-success" onClick={createDetPag}>
              {detailData.length > 0 ? "Actualizar" : "Pagar"}
            </button>
          </div>
        </div>
      ) : (
